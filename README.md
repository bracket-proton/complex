...
用語のローカライズとフロア設定を管理します。`terminology` セクションでUI表示名を、`floors` セクションで各フロアの敵名・色・勝利条件などを定義します。`debug_mode` でデバッグ機能の一括制御が可能です。
...
- `_start_battle()`: フロア設定のロード、敵名とスプライトの設定、デッキの初期化
